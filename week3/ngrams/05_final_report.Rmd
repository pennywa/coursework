...
author: "Penny Wang (CUNY Queens College)"
date: '`r Sys.time()`'
output:
  html_document:
    toc: yes
...
```{r load-counts}
 years <- read_tsv(here("week3", "ngrams", "year_counts.tsv"), 
 col_names = c("term", "year", "volume", "book_count"))

 totals <- read_csv(here("week3", "ngrams", "total_counts.csv"), 
...
"The years dataframe has `r nrow(years)` rows  
 The totals dataframe has `r nrow(totals)` rows."
...
year_proportions <- years %>%
  full_join(totals, by = "year") %>%
  filter(!is.na(term)) %>% 
  mutate(proportion = volume / total_volume)

head(year_proportions)
```
...
plot_data <- year_proportions %>%
  filter(term %in% c(1883, 1910, 1950)) %>%
  filter(year >= 1850 & year <= 2012)

ggplot(plot_data, aes(x = year, y = proportion, color = as.factor(term))) +
  geom_line(linewidth = 0.8) +
  scale_y_continuous(labels = scales::percent) + 
  scale_color_manual(values = c( 
    "1883" = "blue", "1910" = "green", "1950" = "red" )) +
  labs(
    x = "Year",
    y = "Frequency",
    color = "Year"
...
The results here match with the plot presented in the paper in terms of behavior.
There is little to no usage of the term before the year occurs,
There's a spike in frequency of the term that specific year,
Which is followed by a decay as seen in the downward curves for all 3 terms.

I used full_join to combine the raw term frequencies, (years)
with the baseline, (totals)
year is the matching key

full_join is like a union that retains rows from both datasets
filtered by !is.na to keep the rows where the term col is not empty, so no na vals 
mutate to create new proportion col divide the count the term appeared by valome of words printed in that specific year

Used %in%, instead of having to chain together multiple "OR" (|) statements
as.factor(term) becuase R treats it as continous numeric;
This converts it to factor which is categorical so ggplot displays it as 3 distinct categories
scale_color_manual to make it the exact colors as seen from the paper
labels = scales::percent so it mult the decimals by 100 and adds %
labs() replaces the col name as.factor(term) with human readable title "Year"
...
Similar behavior flat at first, year hits  and there's a spike, then followed by decay.
Differences is the ngram viewer is much smoother 
and it looks more like upside down parabolas rather than jagged mountain ridges
...
plot_raw_data <- year_proportions %>%
  filter(term %in% c(1883, 1910, 1950)) %>%
  filter(year >= 1850 & year <= 2012)

ggplot(plot_data, aes(x = year, y = volume, color = as.factor(term))) +
  geom_line(linewidth = 0.8) +
...
```{r plot-totals}
plot_totals_data <- totals %>%
  filter(year >= 1850 & year <= 2012)

ggplot(plot_totals_data, aes(x = year, y = total_volume)) +
  geom_line(linewidth = 0.8, color = "purple") +
  scale_y_continuous(labels = scales::comma) + 
  labs(
    x = "Year",
    y = "Total Word Volume" # all books
  )
```
...
While it looks like the graphs are different, more noticeably with the year 1950 because in part b we see that the shape of 1950 is similar to 1883 and 1910, but in part d its decay behavior is less rapid.
While there’s a difference, we do not have to re-evaluate the results because we have to remember the y axis are different; in part b we’re using proportions but in part d we’re using raw counts. 
This teaches us that when comparing frequencies across eras of different scales, we must normalize our data and really pay attention to the scales or axis labels and what theyre telling us. 
...
```{r compute-peaks}
term_peaks <- year_proportions %>%
  filter(year >= 1850 & year <= 2012) %>%
  filter(term >= 1875 & term <= 1975) %>%
  group_by(term) %>%
    filter(proportion == max(proportion)) %>%
  select(term, peak_year = year, peak_proportion = proportion) %>%
  ungroup()
...
term_half_lives <- year_proportions %>%
  filter(term >= 1875 & term <= 1975) %>%
  left_join(term_peaks, by = "term") %>%
  filter(year >= peak_year) %>%
  filter(proportion <= (peak_proportion / 2)) %>%
  group_by(term) %>%
  filter(year == min(year)) %>%
  mutate(half_life = year - peak_year) %>%
  select(term, peak_year, peak_proportion, half_life_year = year, half_life) %>%
  ungroup()

head(term_half_lives)
```
...
```{r plot-half-lifes}
ggplot(term_half_lives, aes(x = term, y = half_life)) +
  geom_point(alpha = 0.6, color = "black") +
  geom_smooth(color = "purple", linewidth = 1) +
  labs(
    x = "Year",
    y = "Half-life"
  )
...
No, version 2 of NGram data does not produce similar results to version 1 data.
In version 1 data from the inset of figure 3a, there’s a curve but it goes downwards and the datapoints are mostly near or follow the curve.
But with my graph (version 2) there is also a curve but the shape and direction of it is different, it’s more horizontal rather than going down. Also the data points are much more scattered and it doesn’t hug the line as nicely as version 1.
I’m thinking it could be a difference in cleaning because my graph seems to have more noise or perhaps a difference in filtering. 
...
```{r identify-outliers}
#quickly forgotten
shortest_half_lives <- term_half_lives %>%
  arrange(half_life) %>%
  head(3)

print("Years with the shortest half-lives:")
shortest_half_lives

#remembered longest
longest_half_lives <- term_half_lives %>%
...
print("Years with the longest half-lives:")
longest_half_lives
```
...
[1] "Years with the shortest half-lives:"
> shortest_half_lives
# A tibble: 3 × 5
   term peak_year peak_proportion half_life_year half_life
  <dbl>     <dbl>           <dbl>          <dbl>     <dbl>
1  1878      1879       0.0000543           1883         4
2  1877      1879       0.0000473           1883         4
3  1942      1943       0.000167            1947         4


[1] "Years with the longest half-lives:"
> longest_half_lives
# A tibble: 3 × 5
   term peak_year peak_proportion half_life_year half_life
  <dbl>     <dbl>           <dbl>          <dbl>     <dbl>
1  1900      1905       0.0000585           1924        19
2  1910      1911       0.0000794           1929        18
3  1893      1894       0.0000470           1911        17

These were my thoughts before looking at the outliers (to sanity check).
Forgotten quickly: Years before the crisis

Question: Is half life volatile?
Example: Spike in one year -> drops the next year because of data noise? But it bounces back.

Remembered longest:
Years correlated with history
WWI or Great War 
WWII
Great depression
...
The years with the shortest half lives 1877 and 1878 could be due to the end of the Reconstruction Era which was the period that followed the end of the American Civil War. 
1942 was in the middle of WWII so as the saying goes, you tend to remember the beginning and end of things but the middle seems blurry.

For the longest half lives, 1900 was a historical marker (the 20th century). Maybe even 1910 so 1900 and 1910 marks the start of a new era or the turn of the century.
The panic of 1893 was the worst economic depression the US had up to that point, this served as a baseline until the Great Depression in 1929.
